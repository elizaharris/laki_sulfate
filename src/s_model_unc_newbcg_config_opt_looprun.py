...
# Make a wrapper to more easily explore a whole range of scenarios
# Hard part is to define the pathways to explore in terms of where we start, end, mixture...
# 5 parameters: Start config, end config, length of shift - move through these
# Need to make a "light" save version that saves only the ox pathways and the summary
# Some sorts of histograms to illustrate the match between configurations
...
dir = parentdir+'/figs/loops/'+runname
if os.path.exists(dir):
    shutil.rmtree(dir)
os.mkdir(parentdir+'/figs/loops/'+runname)
dir = parentdir+'/output/loops/'+runname
if os.path.exists(dir):
    shutil.rmtree(dir)
os.mkdir(parentdir+'/output/loops/'+runname)
# Save only the summaries in loop mode!! Not iterations
...
fig.tight_layout()
plt.savefig("figs/loops/"+runname+"/background_d34S_D33S_"+runname+".pdf") 
#plt.show()
...
laki_end_depth = [ 59.35, 59.95 ] # Range for the end of the eruption (prev 59.35, 59.65)
...
# Save
laki_mod_mean.to_csv("output/loops/"+runname+"/"+runname+"_"+str(i)+"_model_output.csv")
laki_mod_sd.to_csv("output/loops/"+runname+"/"+runname+"_"+str(i)+"_model_output_sd.csv")
...
my_file = Path("output/output_summary_loops.csv")
if my_file.is_file():
    tmp = pd.read_csv(my_file, sep=',', header=0).drop("Unnamed: 0",axis=1)
    tmp = pd.concat([tmp,this_resid])
    tmp.to_csv("output/output_summary_loops.csv")
else:
    this_resid.to_csv("output/output_summary_loops.csv")
...
fig.tight_layout()
plt.savefig("figs/loops/"+runname+"/model-volc_d34S_d33S_"+runname+".pdf") 
#plt.show()
...
fig.tight_layout()
plt.savefig("figs/loops/"+runname+"/model-params_"+runname+".pdf") 
#plt.show()
